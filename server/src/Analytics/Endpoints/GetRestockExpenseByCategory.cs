...
public class GetRestockExpenseByCategory : IEndpoint
{
    public static void Map(IEndpointRouteBuilder app) => app
        .MapGet("/restock-expense-by-category", Handle)
        .WithSummary("Gets restock expense by category")
        .WithRequestValidation<Request>();

    public record Request(string? Range);
    public class RequestValidator : GetAnalyticsSummary.RequestValidator;
    public record Response(int CategoryId, string CategoryName, decimal RestockExpense);
    ...
    private static async Task<List<Response>> Handle([AsParameters] Request request, AppDbContext database, CancellationToken cancellationToken)
    {
        ...
        var query = database.Transactions.Where(x => x.Type == TransactionType.In);
        ...
        return await query
            ...
            .OrderByDescending(x => x.Sum(t => t.TotalProductCost))
            .Select(x => new Response(
                x.Key.CategoryId,
                x.Key.Name,
                x.Sum(t => t.TotalProductCost)))
